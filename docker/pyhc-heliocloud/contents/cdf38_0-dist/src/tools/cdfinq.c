...
static char *epochToken PROTOARGs((int epoch));
static char *tt2000Token PROTOARGs((int tt2000));
...
{
  QOP *qop;
  static char *validQuals[] = { "id", "page", "nopage", "about", "ziso8601", 
                                NULL };
  static int optRequired[] = { FALSE, FALSE, FALSE, FALSE, FALSE, 0 };
...
void DisplayPadValueDefaults () {
  ...
  snprintf (text, (size_t) sizeof(text),
	    "  CDF_UINT4: ");
  snprintf (EofS(text), (size_t) sizeof(text)-strlen(text),
	    Int32uFORMAT, DEFAULT_UINT4_PADVALUE);
  snprintf (EofS(text), (size_t) sizeof(text)-strlen(text),
	    "\n");
  WriteOutSO (text);
  snprintf (text, (size_t) sizeof(text),
  ...
  WriteOutSO (text);
  dummy = (double) DEFAULT_EPOCH_PADVALUE;
  if (!ziso8601) style = DEFAULTstyleINQ;
  else style = EPOCH3_STYLE;
  epString = EpochDateTimeString (CDF_EPOCH, style, &dummy);
  snprintf (text, (size_t) sizeof(text),
            "  CDF_EPOCH: %s\n", epString);
  if (epString != NULL) free (epString);
  WriteOutSO (text);
  epoch16[0] = epoch16[1] = DEFAULT_EPOCH16_PADVALUE;
  epString = EpochDateTimeString (CDF_EPOCH16, style, epoch16);
  snprintf (text, (size_t) sizeof(text),
            "  CDF_EPOCH16: %s\n", epString);
  if (epString != NULL) free (epString);
  WriteOutSO (text);
...
static void CDFexportInitialVMS () {
  ...
  WriteOut (stdout, text);
  snprintf (text, (size_t) sizeof(text),
	    "              %sEXCLUSIVE,%s,%s,%s,%s,%s,%s%sHEADING)\n",
	    No(DEFAULTexclusiveEXPORT), epochToken(DEFAULTstyleEXPORT),
            epochToken(DEFAULTtt2000EXPORT),
...
static void CDFsimpleInitialVMS () {
  ...
  snprintf (text, (size_t) sizeof(text),
	    ...
	    epochToken(DEFAULTepochSIMPLE),
            epochToken(DEFAULTtt2000SIMPLE),
...
static char *tt2000Token (tt2000Style)
int tt2000Style;
{
  switch (tt2000Style) {
    case TT2000_0_STYLE:
