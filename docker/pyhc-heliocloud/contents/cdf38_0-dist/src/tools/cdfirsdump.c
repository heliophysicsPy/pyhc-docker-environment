     ...
     *************************************************************************/
     switch (recordType) { 
	  ...
	  if (MOST(level)) {
            ...
            if (EPOCHdataType((long)dataType)) {
              snprintf (EofS(text), MAX_SCREENLINE_LEN, "%g",
                        *(double *)valuePtr);
              strcatX (EofS(text), " ==> ", MAX_SCREENLINE_LEN);
            } else if (EPOCH16dataType((long)dataType)) {
              snprintf (EofS(text), MAX_SCREENLINE_LEN, "%g",
                        *(double *)valuePtr);
              strcatX (EofS(text), " ", MAX_SCREENLINE_LEN);
              snprintf (EofS(text), MAX_SCREENLINE_LEN, "%g",
                        *((double *)valuePtr+1));
              strcatX (EofS(text), " ==> ", MAX_SCREENLINE_LEN);
            } else if (TT2000dataType((long)dataType)) {
              snprintf (EofS(text), MAX_SCREENLINE_LEN, "%lld",
                        *(long long *)valuePtr);
              strcatX (EofS(text), " ==> ", MAX_SCREENLINE_LEN);
            }
            if (TT2000dataType((long)dataType)) {
              if (!ziso8601) style = TT2000_3_STYLE;
              else style = TT2000_4_STYLE;
            } else {
              if (!ziso8601) style = DEFAULTstyleIRsDUMP;
            ...
            if (STRINGdataType(dataType) && numElems < 25) {
              int iy;
              char bb[3];
              strcatX (text, " (", MAX_SCREENLINE_LEN);
              for (iy = 0; iy < numElems; ++iy) {
                if (strlen(text) > (MAX_SCREENLINE_LEN-7)) {
                  strcatX (text, "...", MAX_SCREENLINE_LEN);
                  break;
                }
                snprintf (bb, 3, "%02X", (char) *(((char *)valuePtr)+iy)); 
                strcatX (text, bb, MAX_SCREENLINE_LEN);
              }
              strcatX (text, ")", MAX_SCREENLINE_LEN);
            }  
            strcatX (text, "\n", MAX_SCREENLINE_LEN);
     ...
     *************************************************************************/
     switch (recordType) {
	  ...
	  if (MOST(level)) {
	    ...
	    if (!Read4_64(CDFfp,&int32)) return;
	    snprintf (text, (size_t) sizeof(text),
		      "Identifier: %d(%s)\n", (int)int32, (int32==-1?"CDFcore":
                     (int32==1?"JAVA":(int32==2?"Python":"Unknown"))));
	    WriteOut (OUTfp, text);
	    if (!Read4_64(CDFfp,&int32)) return;
	    snprintf (text, (size_t) sizeof(text),
		      "rfuE: %ld\n", (long) int32);
...
OFF_T value;
{
  ...
  static int which = -1;
  which = (which + 1) % 3;
  snprintf (text[which], (size_t) 21, Int64hexFORMAT0x, (long long) value);
  return text[which];
}
...
Int32 value;
{
  ...
  static int which = -1;
  which = (which + 1) % 3;
  snprintf (text[which], (size_t) 11, Int32hexFORMAT0x, value);
  return text[which];
}
