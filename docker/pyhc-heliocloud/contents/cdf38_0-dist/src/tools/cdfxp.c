...
#endif
#if defined(DEBUG)
  if (cdf_FreeMemoryX(NULL,FatalError,1) > 0) DisplayWarning ("Abandoned buffers.");
#else
  cdf_FreeMemoryX (NULL, FatalError,2);
#endif
...
char *argV[];
{
  ...
  ****************************************************************************/
  switch (argC) {
	...
	cdf_FreeMemoryX (qop, FatalError, 3);
	return TRUE;
      }
      /************************************************************************
	...
	delete_pasteboard (ERASE);
	set_cursor_mode (CURSORon);
	cdf_FreeMemoryX (qop, FatalError, 4);
	return TRUE;
      }
      /************************************************************************
      ...
      ***********************************************************************/
      if (!S2qualifierLong(qop,&range,EPOCHRANGEqual,
        ...
        if (range  == 1L) {
          ...
          cdf_FreeMemoryX (epochStrings[0], FatalError, 5);
          cdf_FreeMemoryX (epochStrings[1], FatalError, 6);
        } else if (range == 2L) {
      ...
      if (!S2qualifierLong(qop,&includeVars,INCLUDEqual,
        ...
        if (includeVars > 0 && qop->qualEntered[SETTINGSqual]) {
          ...
          if (includeVars > 0) {
            ...
            cdf_FreeMemoryX (tmp, FatalError, 7);
          }
        }
      }
      /************************************************************************
	...
	**********************************************************************/
	...
	default:
      ...
      ************************************************************************/
      cdf_FreeMemoryX (qop, FatalError, 8);
...
char *varHeader0[];
{
  ...
  ****************************************************************************/
  ...
  for (;;) {
	 ...
	 cdf_FreeMemoryX (CDFs, FatalError, 9);
	 cdf_FreeMemoryX (dirS, FatalError, 10);
	 return;
     }
  }
}
  ...
  for (varX = 0; varX < nVars; varX++) {
     ...
     *************************************************************************/
	 ...
	 case CDF_OK: {
	   ...
	   cdf_FreeMemoryX (buffer, FatalError, 11);
	   Item->filterSetting = opt.eachFilter;
	   break;
	 }
	 case NO_SUCH_ATTR:
     ...
     *************************************************************************/
	 ...
	 case CDF_OK: {
	   ...
	   DisplayStatus (status, readingCDF);
	   if (StatusBAD(status)) {
	     cdf_FreeMemoryX (buffer, FatalError, 12);
	     return FALSE;
	   }
	   ...
	   cdf_FreeMemoryX (buffer, FatalError, 13);
	   Item->filterSetting = opt.eachFilter;
	   break;
	 }
	 case NO_SUCH_ATTR:
     ...
     *************************************************************************/
	 ...
	 cdf_FreeMemoryX (Item->Var->min, FatalError, 14);
	 cdf_FreeMemoryX (Item->Var->max, FatalError, 15);
	 Item->Var->min = NULL;
	 Item->Var->max = NULL;
     ...
     *************************************************************************/
	 ...
	 if (status == BAD_DATA_TYPE)
	   if (Item->Var->fill != NULL) {
	     cdf_FreeMemoryX (Item->Var->fill, FatalError, 16);
             Item->Var->fill = NULL;
           }
       }
/*
	 ...
	 case CDF_OK: {
	   ...
	   DisplayStatus (status, readingCDF);
	   if (StatusBAD(status)) {
	     cdf_FreeMemoryX (buffer, FatalError, 17);
	     return FALSE;
	   }
	   ...
	   cdf_FreeMemoryX (buffer, FatalError, 18);
	   break;
	 }
	 case NO_SUCH_ATTR:
	 case NO_SUCH_ENTRY:
     ...
     *************************************************************************/
	 ...
	 switch (status) {
	   case CDF_OK: {
	     ...
	     DisplayStatus (status, readingCDF);
	     if (StatusBAD(status)) {
	       cdf_FreeMemoryX (buffer, FatalError, 19);
	       return FALSE;
	     }
	     if (STRINGdataType(eDataType)) {
	     ...
	     cdf_FreeMemoryX (buffer, FatalError, 20);
	     break;
	   }
	   case NO_SUCH_ATTR:
	   case NO_SUCH_ENTRY:
	 ...
	 switch (Item->type) {
	   ...
	   case VARIABLEt:
		 ...
		 CatToString (IWsel->iLines[lineN], temp, FORMATitemLEN,
			      LEFT_JUSTIFY, dots);
		 cdf_FreeMemoryX (temp, FatalError, 22);
	       }
	       else
	 ...
	 switch (cX) {
	   ...
	   case MINcx:
		 ...
		 break;
	       case VARIABLEt:
		 if (Item->Var->min != NULL) {
		   cdf_FreeMemoryX (Item->Var->min, FatalError, 23);
		   Item->Var->min = NULL;
	   ...
	   case MAXcx:
		 ...
		 break;
	       case VARIABLEt:
		 if (Item->Var->max != NULL) {
		   cdf_FreeMemoryX (Item->Var->max, FatalError, 24);
		   Item->Var->max = NULL;
	 ...
	 switch (cX) {
	   ...
	   case FILLcx:
		 ...
		 break;
	       case VARIABLEt:
		 if (Item->Var->fill != NULL) {
		   cdf_FreeMemoryX (Item->Var->fill, FatalError, 25);
		   Item->Var->fill = NULL;
		   LoadSelectionWindow (PART3);
		   ItemWindow (UPDATEiw, IWsel, IWsel->itemN);
		 }
	   ...
	   case FORMATcx:
		 ...
		 break;
	       case VARIABLEt:
		 if (Item->Var->format != NULL) {
		   cdf_FreeMemoryX (Item->Var->format, FatalError, 26);
		   Item->Var->format = NULL;
		   LoadSelectionWindow (PART3);
		   ItemWindow (UPDATEiw, IWsel, IWsel->itemN);
		 }
...
void FreeItems () {
  ...
  while (Item != NULL) {
    ...
    switch (Item->type) {
      case RECORDt:
	cdf_FreeMemoryX (Item->Record, FatalError, 27);
	break;
      case INDICESt:
	cdf_FreeMemoryX (Item->Indices, FatalError, 28);
	break;
      case VARIABLEt:
	if (Item->Var->name != NULL) cdf_FreeMemoryX (Item->Var->name,
						 FatalError, 29);
	if (Item->Var->min != NULL) cdf_FreeMemoryX (Item->Var->min,
						FatalError, 30);
	if (Item->Var->max != NULL) cdf_FreeMemoryX (Item->Var->max,
						FatalError, 31);
	if (Item->Var->format != NULL) cdf_FreeMemoryX (Item->Var->format,
						   FatalError, 32);
	if (Item->Var->fill != NULL) cdf_FreeMemoryX (Item->Var->fill,
						 FatalError, 33);
	if (Item->Var->pad != NULL) cdf_FreeMemoryX (Item->Var->pad,
						FatalError, 34);
	if (Item->Var->value != NULL) cdf_FreeMemoryX (Item->Var->value,
						  FatalError, 35);
	cdf_FreeMemoryX (Item->Var, FatalError, 36);
	break;
    }
    cdf_FreeMemoryX (Item, FatalError, 37);
    Item = tItem;
  }
  itemHead = NULL;
  return;
...
{
  int lineN;
  cdf_FreeMemoryX (header[0], FatalError, 38);
  for (lineN = 0; lineN < NUMscreenLINES; lineN++) {
     cdf_FreeMemoryX (scrLines[lineN], FatalError, 39);
  }
  cdf_FreeMemoryX (trailer[0], FatalError, 40);
  return;
}
...
Logical min;
{
  ...
  switch (Item->type) {
    ...
    **************************************************************************/
	...
	return FALSE;
      ...
      ************************************************************************/
      if (NULstring(value)) {
	if (*mmPtr != NULL) {
	  cdf_FreeMemoryX (*mmPtr, FatalError, 41);
	  *mmPtr = NULL;
	...
	cdf_FreeMemoryX (newValue, FatalError, 42);
	return FALSE;
      }
      /************************************************************************
      * Make sure the new minimum is not greater than the current maximum.
	...
	if (GTx(newValue,Item->Var->max,
		Item->Var->dataType,Item->Var->numElems)) {
	  DisplayMessage (minGTmaxMsg, BEEPWAIT1);
	  cdf_FreeMemoryX (newValue, FatalError, 43);
	  return FALSE;
	}
      }
      /************************************************************************
	...
	if (LTx(newValue,Item->Var->min,
		Item->Var->dataType,Item->Var->numElems)) {
	  DisplayMessage (maxLTminMsg, BEEPWAIT1);
	  cdf_FreeMemoryX (newValue, FatalError, 44);
	  return FALSE;
	}
      }
      /************************************************************************
	...
	*mmPtr = cdf_AllocateMemory ((size_t)Item->Var->nValueBytes,
      ...
      cdf_FreeMemoryX (newValue, FatalError, 45);
      Item->filterSetting = TRUE;
      opt.overallFilter = TRUE;
      break;
    }
...
struct VarStructXP *Var;
{
  ...
  ****************************************************************************/
  if (NULstring(value)) {
    if (Var->fill != NULL) {
      cdf_FreeMemoryX (Var->fill, FatalError, 46);
      Var->fill = NULL;
      return TRUE;
    }
    else
  ...
  ****************************************************************************/
  ...
  if (numElems != Var->numElems) {
    ...
    cdf_FreeMemoryX (newValue, FatalError, 47);
    return FALSE;
  }
  /****************************************************************************
  * Save new value.
  ****************************************************************************/
  ...
  cdf_FreeMemoryX (newValue, FatalError, 48);
  opt.useFills = TRUE;
  return TRUE;
}
...
struct ItemStruct *Item;
{
  ...
  ****************************************************************************/
  if (NULstring(format)) {
    if (Item->Var->format != NULL) {
      cdf_FreeMemoryX (Item->Var->format, FatalError, 49);
      Item->Var->format = NULL;
      return TRUE;
    }
    else
...
void FreeTemps ()
{
  ...
  if (numVars > 0) {
    for (i = 0; i < numVars; ++i)
      if (vars[i] != NULL) cdf_FreeMemoryX  (vars[i], FatalError, 52);
    cdf_FreeMemoryX (vars, FatalError, 53);
  }
  if (numEpochs > 0) {
