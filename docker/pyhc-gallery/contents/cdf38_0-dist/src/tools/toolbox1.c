...
static long GetCdataType PROTOARGs((char *, long, long *));
...
static void RemoveExtra0s PROTOARGs((char *));
  ...
  ****************************************************************************/
  if (*numDims > 0) {
    ...
    for (dimN = 0; dimN <= lastDim; dimN++) {
       ...
       if (dimSizes[dimN] <= 0) return FALSE;
       cp = NextNonDigit (cp);
       cp = NextNonSpace (cp);
       if (dimN != lastDim) {
	 if (*cp != ',') return FALSE;
...
char *format;
{
  ...
  ****************************************************************************/
  ...
  if (strstr(ptr, "Z") != NULL || strstr(ptr, "z") != NULL ||
      strstr(ptr, "X") != NULL || strstr(ptr, "X") != NULL) width = width + 2;
  return width;
}
  ...
  ****************************************************************************/
  if (EPOCHdataType(dataType)) {
    ...
    if (format == NULL)
      ...
      if (IsFloatingValidValue(dataType, binary) == 0)
	EncodeValue (dataType, binary, tText, 0, width);
      else {  
	if (strchr(format,'%') != NULL)
	  if (GetCdataType(format,dataType,&CdataType))
...
size_t width;		/* In: size of encoded text. */
{
  ...
  switch (dataType) {
    ...
    case CDF_EPOCH:
      ...
      break;
    case CDF_EPOCH16:
      if (isnan(*(double *) binary) || isnan(*(((double *) binary)+1))) {
        snprintf (text, 4, "%s", "NaN");
      } else {
        switch (style) {
          case EPOCH0_STYLE:
    ...
    case CDF_TIME_TT2000:
      ...
      switch (style) {
        case TT2000_0_STYLE:
          encodeTT2000 (*(long long *) binary, text, 0);
          break;
        case TT2000_1_STYLE:
          encodeTT2000 (*(long long *) binary, text, 1);
          break;
        case TT2000_2_STYLE:
          encodeTT2000 (*(long long *) binary, text, 2);
          break;
        case TT2000_3_STYLE:
          encodeTT2000 (*(long long *) binary, text, 3);
          break;
        case TT2000_4_STYLE:
          encodeTT2000 (*(long long *) binary, text, 4);
          break;
      }
      break;
  }
#if defined(win32)
  if (FLOAT4dataType(dataType) || DOUBLEdataType(dataType))
...
/******************************************************************************
* EncodeValueSKT.
* Returns width of encoded value form cdf2stk tool.
******************************************************************************/

int EncodeValueSKT (dataType, binary, text, style, width)
long dataType;          /* CDF data type. */
void *binary;           /* In: Value to encode. */
char *text;             /* Out: Encoded value. */
int style;              /* In: EPOCH style. */
  ...
  switch (dataType) {
	...
	char *ePtr, tText[80+1];
	  ...
	  snprintf (text, width, "%s%s",
          ...
          if (*(float *) binary <= DEFAULT_FLOAT_PADVALUE ||
              *(float *) binary >= (0.0f - DEFAULT_FLOAT_PADVALUE) ||
              (*(float *) binary >= (float) -1.0e-4 &&
               *((float *) binary) <= (float) 1.0e-4))
            snprintf (text, width, "%g", *(float *) binary);
          else
            snprintf (text, width, "%f", *(float *) binary);
        }
#else
        if (isinf((double)*(float *) binary))
	  snprintf (text, width, "%s%s",
	...
	char *ePtr, tText[80+1];
	  ...
	  snprintf (text, width, "%s%s",
          ...
          if (*(double *) binary <= DEFAULT_DOUBLE_PADVALUE ||
              *(double *) binary >= (0.0 - DEFAULT_DOUBLE_PADVALUE) ||
              (*(double *) binary >= -1.0e-4 && *(double *) binary <= 1.0e-4))
            snprintf (text, width, "%g", *(double *) binary);
          else
	    snprintf (text, width, "%f", *(double *) binary);
        }
#else
        if (isinf(*((double *) binary)))
	  snprintf (text, width, "%s%s",
			   ...
			   no maximum width (no truncating). */
{
  ...
  * Encode string.
  ****************************************************************************/
  numBytes = (long) UTF8StrLength ((unsigned char *)iString, (int)nChars);
...
size_t width;		/* Size of encoded text. */
{
  switch (dataType) {
	...
	case CDF_INT8:
	  ...
	  break;
	case CDF_UINT4:
	  if (strstr(Cformat, "X") != NULL || strstr(Cformat, "x") != NULL)
	    snprintf (text, width, Cformat, (uInt32) *((unsigned int *) binary));
	  else
	...
	case CDF_INT8:
	  if (strstr (Cformat, "X") != NULL || strstr (Cformat, "x") != NULL)
	    snprintf (text, width, Cformat, (Int64) *((long long *) binary));
	  else
...
#if defined(win32)
  if (FLOAT4dataType(CdataType) || DOUBLEdataType(CdataType))
    RemoveLeading0 (text, width);
#endif
  if (IsHexFormat(Cformat) && (text[0] != '0' || (text[1] != 'x' && 
                                                  text[1] != 'X'))) {
    char tmp[MAX_nonSTRING_VALUE_LEN+1]; int ix;
    int len = (int) strlen(text);
    strcpyX (tmp, "0x", MAX_nonSTRING_VALUE_LEN);
    if (dataType == CDF_INT1 || dataType == CDF_BYTE ||
        dataType == CDF_UINT1) {
      if (len < 2)
        strcatX (tmp, "0", MAX_nonSTRING_VALUE_LEN);
    } else if (dataType == CDF_INT2 || dataType == CDF_UINT2) {
      if (len < 4) {
        for (ix = 0; ix < (4 - len); ++ix)
          strcatX (tmp, "0", MAX_nonSTRING_VALUE_LEN);
      }
    } else if (dataType == CDF_INT4 || dataType == CDF_UINT4 ||
               dataType == CDF_REAL4 || dataType == CDF_FLOAT) {
      if (len < 8) {
        for (ix = 0; ix < (8 - len); ++ix)
          strcatX (tmp, "0", MAX_nonSTRING_VALUE_LEN);
      }
    } else if (dataType == CDF_INT8 || dataType == CDF_REAL8 ||
               dataType == CDF_DOUBLE) {
      if (len < 16) {
        for (ix = 0; ix < (16 - len); ++ix)
          strcatX (tmp, "0", MAX_nonSTRING_VALUE_LEN);
      }
    }
    strcatX (tmp, text, MAX_nonSTRING_VALUE_LEN);
    strcpyX (text, tmp, width);
  }
  return strlen(text);
}
    ...
    **************************************************************************/
	...
	case 0:
	  switch (dataType) {
	    ...
	    case CDF_INT8:
#if defined(alphaosf)
	      strcpyX (Cformat, "0x%016X", width);
#else
	      strcpyX (Cformat, "0x%016llX", width);
#endif
	      *CdataType = CDF_INT8;
	      return TRUE;
	    case CDF_UINT4:
	...
	case 1:
	  switch (dataType) {
	    case CDF_BYTE:
	    case CDF_INT1:
	      snprintf (Cformat, width, "0x%%%d.%dX", w, w);
	      *CdataType = CDF_UINT1;
	      return TRUE;
	    case CDF_UINT1:
	      snprintf (Cformat, width, "0x%%%d.%dX", w, w);
	    ...
	    case CDF_UINT2:
	      ...
	      *CdataType = CDF_UINT2;
	      return TRUE;
	    case CDF_INT4:
	      snprintf (Cformat, width, "0x%%%d.%d%sX", w, w, Int32FORMATmod);
	      *CdataType = CDF_UINT4;
	      return TRUE;
	    case CDF_INT8:
	      snprintf (Cformat, width, "0x%%%d.%d%sX", w, w, Int64FORMATmod);
	      *CdataType = CDF_INT8;
	      return TRUE;
	    case CDF_UINT4:
	      snprintf (Cformat, width, "0x%%%d.%d%sX", w, w, Int32FORMATmod);
	...
	case 2:
	  switch (dataType) {
	    case CDF_BYTE:
	    case CDF_INT1:
	      snprintf (Cformat, width, "0x%%%d.%dX", w, m);
	      *CdataType = CDF_UINT1;
	      return TRUE;
	    case CDF_UINT1:
	      snprintf (Cformat, width, "0x%%%d.%dX", w, m);
	    ...
	    case CDF_UINT2:
	      ...
	      *CdataType = CDF_UINT2;
	      return TRUE;
	    case CDF_INT4:
	      snprintf (Cformat, width, "0x%%%d.%d%sX", w, m, Int32FORMATmod);
	      *CdataType = CDF_UINT4;
	      return TRUE;
	    case CDF_INT8:
	      snprintf (Cformat, width, "0x%%%d.%d%sX", w, m, Int64FORMATmod);
	      *CdataType = CDF_INT8;
	      return TRUE;
	    case CDF_UINT4:
	      snprintf (Cformat, width, "0x%%%d.%d%sX", w, m, Int32FORMATmod);
...
static long GetCdataType (Cformat, dataType, CdataType)
char *Cformat;          /* C format specification. */
long dataType;          /* Original data type. */
...
#  if !defined(win32)
      ...
      strcpyX (path, dest, (size_t) (lastdel-dest));
...
/******************************************************************************
* NulPadRight.
*   Concatenate Nuls to the right end of a string.
******************************************************************************/

void NulPadRight (string, count)
char *string;
int count;
{
  int i;
  for (i = 0; i < count; i++) catchrX (string, (int) '\0', 0);
  return;
}
...
/******************************************************************************
* RemoveExtra0s.
* For a floating point value, this removes extra 0s at the end of precision.
******************************************************************************/

void RemoveExtra0s (text)
char *text;		/* String to work on... */
{
  char *ptr; int ix;
  ptr = strchr (text, '.');
  if (ptr == NULL) return;
  ix = (int) strlen (text);
  if (text[ix-1] != '0') return;
  while (text[ix-1] == '0') --ix;
  if (ix == (int) (ptr - text)) ++ix;
  text[ix+1] = (char) '\0';
}
   ...
   *(buf+numElems) = '\0';
   ...
   free (buf);
    ...
    if (dataType == CDF_EPOCH) {
      ...
      if (isnan(*(double *)data) || isnan(*(((double *)data)+1))) {
        printf("  NaN ");
      } else {
...
/******************************************************************************
* EpochDateTimeString. Based on epoch's data type, CDF_EPOCH or CDF_EPOCH16,
* and encoded style, this function returns its date/time string.
******************************************************************************/
char *EpochDateTimeString (long dataType, int style, double *data) {

  char *epString = NULL;
  if (dataType != CDF_EPOCH && dataType != CDF_EPOCH16) return epString;
  if (dataType == CDF_EPOCH) {
    switch (style) {
      case (EPOCH0_STYLE): {
        epString = (char *) malloc (sizeof(char) * (EPOCH_STRING_LEN + 1));
        encodeEPOCH (*data, epString);
        break;
      }
      case (EPOCH1_STYLE): {
        epString = (char *) malloc (sizeof(char) * (EPOCH1_STRING_LEN + 1));
        encodeEPOCH1 (*data, epString);
        break;
      }
      case (EPOCH2_STYLE): {
        epString = (char *) malloc (sizeof(char) * (EPOCH2_STRING_LEN + 1));
        encodeEPOCH2 (*data, epString);
        break;
      }
      case (EPOCH3_STYLE): {
        epString = (char *) malloc (sizeof(char) * (EPOCH3_STRING_LEN + 1));
        encodeEPOCH3 (*data, epString);
        break;
      }
      case (EPOCH4_STYLE): {
        epString = (char *) malloc (sizeof(char) * (EPOCH4_STRING_LEN + 1));
        encodeEPOCH4 (*data, epString);
        break;
      }
      default:
        epString = (char *) malloc (sizeof(char) * 2);
        strcpyX (epString, "?", 1); 
    }
  } else {
    switch (style) {
      case (EPOCH0_STYLE): {
        epString = (char *) malloc (sizeof(char) * (EPOCH16_STRING_LEN + 1));
        encodeEPOCH16 (data, epString);
        break;
      }
      case (EPOCH1_STYLE): {
        epString = (char *) malloc (sizeof(char) * (EPOCH16_1_STRING_LEN + 1));
        encodeEPOCH16_1 (data, epString);
        break;
      }
      case (EPOCH2_STYLE): {
        epString = (char *) malloc (sizeof(char) * (EPOCH16_2_STRING_LEN + 1));
        encodeEPOCH16_2 (data, epString);
        break;
      }
      case (EPOCH3_STYLE): {
        epString = (char *) malloc (sizeof(char) * (EPOCH16_3_STRING_LEN + 1));
        encodeEPOCH16_3 (data, epString);
        break;
      }
      case (EPOCH4_STYLE): {
        epString = (char *) malloc (sizeof(char) * (EPOCH16_4_STRING_LEN + 1));
        encodeEPOCH16_4 (data, epString);
        break;
      }
      default:
        epString = (char *) malloc (sizeof(char) * 2);
...
* GetDefaultPadValue. 
******************************************************************************/
void GetDefaultPadValue (CDFid id, long dataType, void *value) {
  ...
  status = CDFlib (SELECT_, CDF_, id,
                   ...
                   NULL_);
  if (version*10000+release*100+increment >= 30500) {
    /* CDF V3.5.0 and after */
    switch (dataType) {
      case CDF_BYTE:
        *(sChar *) value = (sChar) DEFAULT_BYTE_PADVALUE;
        break;
      case CDF_INT1:
        *(sChar *) value = (sChar) DEFAULT_INT1_PADVALUE;
        break;
      case CDF_UINT1:
        *(uChar *) value = (uChar) DEFAULT_UINT1_PADVALUE;
        break;
      case CDF_INT2:
        *(Int16 *) value = (Int16) DEFAULT_INT2_PADVALUE;
        break;
      case CDF_UINT2:
        *(uInt16 *) value = (uInt16) DEFAULT_UINT2_PADVALUE;
        break;
      case CDF_INT4:
        *(Int32 *) value = (Int32) DEFAULT_INT4_PADVALUE;
        break;
      case CDF_UINT4:
        *(uInt32 *) value = (uInt32) DEFAULT_UINT4_PADVALUE;
        break;
      case CDF_INT8:
        *(Int64 *) value = (Int64) DEFAULT_INT8_PADVALUE;
        break;
      case CDF_REAL4:
        *(float *) value = (float) DEFAULT_REAL4_PADVALUE;
        break;
      case CDF_FLOAT:
        *(float *) value = (float) DEFAULT_FLOAT_PADVALUE;
        break;
      case CDF_REAL8:
        *(double *) value = (double) DEFAULT_REAL8_PADVALUE;
        break;
      case CDF_DOUBLE:
        *(double *) value = (double) DEFAULT_DOUBLE_PADVALUE;
        break;
      case CDF_EPOCH:
        *(double *) value = (double) DEFAULT_EPOCH_PADVALUE;
        break;
      case CDF_EPOCH16: {
        *((double *) value) = (double) DEFAULT_EPOCH16_PADVALUE;
        *(((double *) value) + 1) = (double) DEFAULT_EPOCH16_PADVALUE;
        break;
      }
      case CDF_TIME_TT2000:
        *(long long *) value = (long long) DEFAULT_TT2000_PADVALUE;
        break;
      case CDF_CHAR:
        *(sChar *) value = (sChar) DEFAULT_CHAR_PADVALUE;
        break;
      case CDF_UCHAR:
        *(uChar *) value = (uChar) DEFAULT_UCHAR_PADVALUE;
        break;
      default:
        break;
    }
    ...
    switch (dataType) {
      ...
      case CDF_UINT4:
        *(uInt32 *) value = (uInt32) 0;
        break;
      case CDF_INT8:
        *(Int64 *) value = (Int64) DEFAULT_INT8_PADVALUE - 1;
        break;
      case CDF_REAL4:
...
* ReplacePadValues. 
******************************************************************************/
void ReplacePadValues (void *buffer, void *pad, void *filled, 
                       long dataType, int values) {
  int ix;
  switch (dataType) {
    case CDF_BYTE:
      for (ix = 0; ix < values; ++ix)
        if (*(((sChar *)buffer)+ix) == *(sChar *)pad)
          *(((sChar *)buffer)+ix) = *(sChar *)filled;
      break;
    case CDF_INT1:
      for (ix = 0; ix < values; ++ix)
        if (*(((sChar *)buffer)+ix) == *(sChar *)pad)
    ...
    case CDF_UINT1:
      for (ix = 0; ix < values; ++ix)
        if (*(((uChar *)buffer)+ix) == *(uChar *)pad)
          *(((uChar *)buffer)+ix) = *(uChar *)filled;
      break;
    case CDF_INT2:
      for (ix = 0; ix < values; ++ix)
        if (*(((Int16 *)buffer)+ix) == *(Int16 *)pad)
          *(((Int16 *)buffer)+ix) = *(Int16 *)filled;
      break;
    case CDF_UINT2:
      for (ix = 0; ix < values; ++ix)
        if (*(((uInt16 *)buffer)+ix) == *(uInt16 *)pad)
          *(((uInt16 *)buffer)+ix) = *(uInt16 *)filled;
      break;
    case CDF_INT4:
      for (ix = 0; ix < values; ++ix)
        if (*(((Int32 *)buffer)+ix) == *(Int32 *)pad)
          *(((Int32 *)buffer)+ix) = *(Int32 *)filled;
      break;
    case CDF_UINT4:
      for (ix = 0; ix < values; ++ix)
        if (*(((uInt32 *)buffer)+ix) == *(uInt32 *)pad)
          *(((uInt32 *)buffer)+ix) = *(uInt32 *)filled;
      break;
    case CDF_INT8:
      for (ix = 0; ix < values; ++ix)
        if (*(((Int64 *)buffer)+ix) == *(Int64 *)pad)
          *(((Int64 *)buffer)+ix) = *(Int64 *)filled;
      break;
    case CDF_REAL4:
      for (ix = 0; ix < values; ++ix)
        if (*(((float *)buffer)+ix) == *(float *)pad)
          *(((float *)buffer)+ix) = *(float *)filled;
      break;
    case CDF_FLOAT:
      for (ix = 0; ix < values; ++ix)
        if (*(((float *)buffer)+ix) == *(float *)pad)
    ...
    case CDF_REAL8:
      for (ix = 0; ix < values; ++ix)
        if (*(((double *)buffer)+ix) == *(double *)pad)
          *(((double *)buffer)+ix) = *(double *)filled;
      break;
    case CDF_DOUBLE:
      for (ix = 0; ix < values; ++ix)
        if (*(((double *)buffer)+ix) == *(double *)pad)
    ...
    case CDF_EPOCH16:
      for (ix = 0; ix < values; ++ix) {
        if (*(((double *)buffer)+2*ix) == *(double *)pad)
          *(((double *)buffer)+2*ix) = *(double *)filled;
        if (*(((double *)buffer)+2*ix+1) == *(((double *)pad)+1))
          *(((double *)buffer)+2*ix+1) = *(((double *)filled)+1);
      }
      break;
    case CDF_TIME_TT2000:
      for (ix = 0; ix < values; ++ix)
        if (*(((long long *)buffer)+ix) == *(long long *)pad)
          *(((long long *)buffer)+ix) = *(long long *)filled;
      break;
    case CDF_CHAR:
      for (ix = 0; ix < values; ++ix)
        if (*(((sChar *)buffer)+ix) == *(sChar *)pad)
...
CDFstatus GetFORMATEntry (id, Z, varNum, format)
CDFid id;
Logical Z;
long varNum;
char **format;
{
  CDFid curId;
  CDFstatus status, status2; long dataType, numElems, curAttr, curEntry;
  curId = (CDFid) -1;
  curAttr = curEntry = -1L;
  status2 = CDFlib (CONFIRM_, CDF_, &curId,
                    NULL_);
  ...
  if (status != CDF_OK) {
    *format = NULL;
    if (curId != (CDFid) -1) 
      status2 = CDFlib (SELECT_, CDF_, curId,
                        NULL_);
    if (curAttr != -1L) 
